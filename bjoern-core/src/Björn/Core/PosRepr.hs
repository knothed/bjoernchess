module Björn.Core.PosRepr where
...
allPieces pos = pcs White ++ pcs Black where
    pcs col = pc Björn [björn pos col] ++ pc King (toList (king pos col)) ++ map pawn (pawns pos col) where
        pc p pcs = map (\sq -> (p, sq, col)) pcs
        pawn (sq, b) = (Pawn b, sq, col)

-- Check whether a position representation is valid, meaning:
-- No björns or kings are adjacent to each other, no field is occupied twice and no piece has an invalid position.
...
posValid pos = nonAdjacent && noMultiOccupation && piecesOnBoard where
    nonAdjacent = dist (björn pos White) (björn pos Black) > 1 &&
